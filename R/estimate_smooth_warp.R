# TODO: NUMERICAL ISSUES WITH MATERN COVARIANCE
...
estimate_smooth_warp <- function(y, t, amp_cov_par, amp_cov_fct, warp_cov_par, warp_cov_fct, kts, tw, intercept = FALSE, iter = c(5, 5), use_warp_gradient = FALSE, homeomorphisms = 'no', like_optim_control = list()) {
  nouter <- iter[1] + 1
  ninner <- iter[2]
  ...
  c <- spline_weights(y, t, w, tw, bdiag(Ainv), kts, intercept = intercept, smooth_warp = TRUE)
...
#             gr <- function(w, t, y, tw, c, Ainv, Cinv, kts, intercept) posterior_grad(w, dwarp[[i]], t, y, tw, c, Ainv, Cinv, kts, intercept, )
          ...
          w[, i] <- optim(par = w[,i], fn = posterior, gr = gr, method = warp_optim_method, t = t[[i]], y = y[[i]], tw = tw, c = c, Ainv = Ainv[[i]], Cinv = Cinv, kts = kts, intercept = intercept, smooth_warp = TRUE)$par
